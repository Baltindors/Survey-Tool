  ...
  >
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col">
          ...
          <select 
            ...
            class="text-xs font-bold text-slate-700 bg-transparent border-none p-0 focus:ring-0 cursor-pointer outline-none"
          >
        ...
        <thead>
          <tr>
            ...
            <th v-for="(col, cIdx) in question.cols" :key="cIdx" class="p-1 group relative">
              ...
              />
              <button @click="removeCol(cIdx)" class="absolute -top-1 right-0 opacity-0 group-hover:opacity-100 text-red-400"><Trash2 :size="10"/></button>
            </th>
            <th><button @click="addCol" class="p-2 text-blue-500 hover:bg-blue-50 rounded"><Plus :size="14"/></button></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(row, rIdx) in question.rows" :key="rIdx" class="group">
            <td class="py-2 pr-2">
              <div class="flex items-center gap-1">
                <button @click="removeRow(rIdx)" class="opacity-0 group-hover:opacity-100 text-red-300"><Trash2 :size="12"/></button>
                <input 
                  :value="row"
    ...
    <div v-else class="space-y-2">
      <div v-for="(opt, oIdx) in question.cols" :key="oIdx" class="flex items-center gap-3 group bg-gray-50/50 p-2 rounded-lg border border-transparent hover:border-blue-100">
        ...
        />
        <button @click="removeCol(oIdx)" class="opacity-0 group-hover:opacity-100 text-red-300 hover:text-red-500"><Trash2 :size="14"/></button>
      </div>
      <button 
        @click="addCol"
